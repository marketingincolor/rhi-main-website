...
if (isset($_REQUEST['action'])) {
    switch ($_REQUEST['action']) {
        case 'map_import':
            usa_html5map_plugin_import();
            break;
        case 'new':
            $name      = sanitize_text_field($_REQUEST['name']);
            $options[] = usa_html5map_plugin_map_defaults($name);
            $update    = true;
            break;
        case 'delete':
...
if ($update) update_site_option('usahtml5map_options',$options);
    ...
    private function table_data()
    {
        ...
        foreach ($options as $map_id => $map_data) {
            $data[] = array(
                            ...
                            'edit'      => '<a href="admin.php?page=usa-html5-map-options&map_id='.$map_id.'">'.__('General settings', 'usa-html5-map').'</a><br />
                                            <a href="admin.php?page=usa-html5-map-states&map_id='.$map_id.'">'.__('Detailed settings', 'usa-html5-map').'</a><br />
                                            <a href="admin.php?page=usa-html5-map-view&map_id='.$map_id.'">'.__('Preview', 'usa-html5-map').'</a><br /><br />
...
?>
    ...
    <div class="wrap">
        ...
        <h2><?php echo __('HTML5 Maps', 'usa-html5-map'); ?></h2>
        <?php $listtable->display(); ?>
    </div>
    
    <form name="action_form" action="" method="POST" enctype="multipart/form-data" class="usa-html5-map full">
